...
if (-not $sessions) {
    Write-Host 'No WinUI3 control-plane sessions found.'
    exit 0
}

foreach ($session in $sessions) {
    try {
        $state = & "$PSScriptRoot\winui3-control-send.ps1" -SessionName $session.Session -Type STATE 2>$null
            ...
            for ($i = 5; $i -lt $parts.Length; $i++) {
                if ($parts[$i] -match '^(?<k>[^=]+)=(?<v>.*)$') {
                    switch ($Matches.k) {
                        ...
                        'tab_count' { $session.TabCount = $Matches.v }
                        'active_tab' { $session.ActiveTab = $Matches.v }
                    }
                }
            }
        }
...
$sessions | Format-Table Session,Pid,Hwnd,Prompt,Selection,Pwd,TabCount,ActiveTab,Title,Pipe,Log -AutoSize
