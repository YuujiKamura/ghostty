...
const log = std.log.scoped(.win32_control_plane);
...
pub const ControlPlane = struct {
    ...
    pub const CaptureStateFn = *const fn (ctx: *anyopaque, allocator: Allocator) anyerror!?StateSnapshot;
    pub const CaptureTailFn = *const fn (ctx: *anyopaque, allocator: Allocator) anyerror!?[]u8;
    ...
    pub fn isEnabled(allocator: Allocator) bool {
        const value = std.process.getEnvVarOwned(allocator, "GHOSTTY_WIN32_CONTROL_PLANE") catch return false;
    ...
    pub fn create(
        ...
        const root_dir = try std.fs.path.join(allocator, &.{ local_appdata, "ghostty", "control-plane", "win32" });
        ...
        const pipe_name = try std.fmt.allocPrint(allocator, "ghostty-win32-{s}-{d}", .{ safe_session_name, pid });
    ...
    fn buildResponse(self: *ControlPlane, request: []const u8) ![]u8 {
        ...
        if (std.mem.eql(u8, request, "STATE")) {
            ...
            if (self.capture_state_fn) |capture| {
                if (self.callback_ctx) |ctx| {
                    snapshot = capture(ctx, self.allocator) catch |err| blk: {
            ...
            return std.fmt.allocPrint(
                self.allocator,
                "STATE|{s}|{d}|0x{x}|{s}|prompt={d}|selection={d}|pwd={s}\n",
                .{ self.session_name, self.pid, @intFromPtr(self.hwnd), title, prompt, selection, pwd },
            );
        }
        ...
        if (std.mem.startsWith(u8, request, "INPUT|")) {
            ...
            defer self.allocator.free(line);
            self.appendLog(line);
            _ = os.PostMessageW(self.hwnd, os.WM_GHOSTTY_CONTROL_INPUT, 0, 0);
            return std.fmt.allocPrint(self.allocator, "ACK|{s}|{d}\n", .{ self.session_name, self.pid });
        }
    ...
    fn buildTailResponse(self: *ControlPlane, requested_lines: usize) ![]u8 {
        ...
        if (self.capture_tail_fn) |capture| {
            if (self.callback_ctx) |ctx| {
                const viewport = capture(ctx, self.allocator) catch |err| blk: {
    ...
    fn inferPromptFromViewport(self: *ControlPlane, pwd: []const u8, title: []const u8) !bool {
        ...
        const viewport = (capture(ctx, self.allocator) catch |err| {
...
fn loadSessionName(allocator: Allocator, pid: u32) ![]u8 {
    ...
    return std.fmt.allocPrint(allocator, "win32-{d}", .{pid});
}
