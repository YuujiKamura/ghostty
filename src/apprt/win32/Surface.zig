/// Win32 Surface implementation for Ghostty.
const Surface = @This();
...
pub fn init(self: *Surface, app: *App, core_app: *CoreApp, config: *const configpkg.Config) !void {
    ...
    log.info("Win32 surface initialized ({d}x{d})", .{ self.size.width, self.size.height });
}
...
pub fn close(self: *Surface, process_active: bool) void {
    _ = process_active;
    // In MVP, closing the surface closes the app
...
pub fn clipboardRequest(
    ...
    // Only standard clipboard is supported on Win32.
...
pub fn handleCharEvent(self: *Surface, char_code: u16) void {
    ...
    const event: input.KeyEvent = switch (state) {
        .pending => |pk| .{
            .action = .press,
            .key = pk.key_code,
            .mods = pk.mods,
