...
//!
//! Hecker's PR replaced the XAML TextBox-based IME approach with direct TSF
//! integration. The key architectural decisions carried over to this port:
//!
//!   1. **Direct TSF on the HWND, not on a XAML TextBox.**
//!      The TextBox approach suffered from focus races, layout desync, and
//!      composition artifacts. TSF is associated directly with the top-level
//!      HWND via ITfThreadMgrEx, bypassing XAML's input stack entirely.
//!
//!   2. **EditSessionProxy for async composition extraction.**
//!      TSF requires edit sessions (ITfEditSession::DoEditSession) to read
//!      composition state. WT wraps these in a proxy template; we use an
//!      equivalent inline COM object (EditSessionProxy).
//!
//!   3. **GUID_PROP_COMPOSING + GUID_PROP_ATTRIBUTE range walking.**
//!      doCompositionUpdate enumerates ranges via TrackProperties to separate
//!      finalized text (composing=false, at the start) from active composition
//!      text (composing=true). This matches WT's _doCompositionUpdate exactly.
//!
//!   4. **Callback-driven output (handleOutput / handlePreedit).**
//!      Finalized text is delivered via _handleOutput; active composition text
//!      via _handlePreedit. The terminal apprt wires these to the PTY writer
//!      and preedit renderer respectively.
//!
//! ## Zig-specific adaptations
//!
//!   - COM interfaces use hand-written vtables (no C++, no midl) generated by
//!     win-zig-bindgen. @fieldParentPtr recovers `self` from vtable pointers.
//!   - IEnumTfPropertyValue is defined inline (not in standard bindings).
//!   - VARIANT handling uses raw byte layouts matching the Win64 ABI.
//!
//! ## License
//!
//! Windows Terminal is licensed under the MIT License. Copyright (c) Microsoft
//! Corporation. The original TSF implementation was written by Leonard Hecker.
