...
pub const StringList = struct {
    ...
    pub fn create(alloc: Allocator, strings: []const [:0]const u8) Allocator.Error!*StringList {
    ...
    pub fn boxedCopy(self: *StringList) Allocator.Error!*StringList {
        return StringList.create(self.arena.child_allocator, self.strings);
    }

    pub fn boxedFree(self: *StringList) void {
        self.destroy();
    }
};
...
test "StringList boxedCopy and boxedFree" {
    ...
    const copied = try original.boxedCopy();
    defer copied.boxedFree();
