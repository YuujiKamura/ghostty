...
/// Called via LayoutUpdated on the TabView's FrameworkElement.
pub fn onLayoutUpdated(self: anytype, _: ?*anyopaque, _: ?*anyopaque) void {
    const nci = self.nci_window orelse return;
    const hwnd = self.hwnd orelse return;

    var rect: @import("os.zig").RECT = .{};
    _ = @import("os.zig").GetClientRect(hwnd, &rect);
    nci.resizeDragBarWindow(rect.right - rect.left);
}
