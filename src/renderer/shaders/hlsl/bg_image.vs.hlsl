...
struct VSOutput {
    ...
    // bools aren't allowed for vertex outputs.
...
VSOutput vs_main(VSInput input) {
    ...
    if (fit == BG_IMAGE_CONTAIN) {
        ...
        // For `none` (BG_IMAGE_NO_FIT) we just use the original texture size.
        dest_size = tex_size;
    }

    float2 start = float2(0.0, 0.0);
    float2 mid = (scr_size - dest_size) / float2(2.0, 2.0);
    float2 end = scr_size - dest_size;
